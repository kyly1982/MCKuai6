		...
		OnItemClickListener, IXListViewListener
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		mOptions = application.getCircleOptions();
	}
	...
	private void showPostList()
	{
		...
		if (null != posts && posts.size() != 0)
		{
			if (null == postAdapter)
			{
				postAdapter = new PostAdapter(SearchResultActivity.this, posts);
	...
	private void showUserList()
	{
		...
		} else if (0 == users.size())
		{
			if (null != searchContext)
			{
				showNotification(1,"你还没有添加有好友！\n点其头像可以进入个人中心然后再添加好友。",R.id.root);
		...
		mClient.post(url, params, new JsonHttpResponseHandler()
		{
			...
			public void onStart()
			{
				...
				if (null == searchContext && !isHandlePost && null == userData)
				{
					if (null == userData)
					{
						String result = MyApplication.getInstance().getFriends();
						if (null != result)
						{
							parseResult(result);
							return;
	...
	private void parseResult(String result)
	{
		if (isHandlePost)
		{
			postData = gson.fromJson(result, PostBean.class);
...
//					showNotification("选中用户后才能进行此操作!");
	...
	private String getUrl()
	{
		...
		{
			url += isHandlePost ? getString(R.string.interface_collect) : getString(R.string.interface_fellow);
		}
		return url;
	}
